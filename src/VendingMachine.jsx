...
const vmContractAddress = "0x0D17C16BB7bD73Ef868cD452A1B10D069685249f"; // Адрес смарт-контракта
const abi = [
...
function VendingMachine({ provider }) {
  ...
  async function purchaseCupcakes(evt) {
    ...
    try {
      ...
      const tx = await writableContract.purchase(purchaseCups, {
  ...
  return (
    <div className="container">
      ...
      <form>
        ...
        <label>
          Purchase Cupcakes
          <input placeholder="1, 2, 3..." value={purchaseCups} onChange={(e) => setPurchaseCups(e.target.value)} />
        </label>
        <input type="submit" className="button" value="Purchase" onClick={purchaseCupcakes} disabled={!writableContract} />
      </form>
